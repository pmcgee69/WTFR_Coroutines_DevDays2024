...
begin
  ...
  for j := 0 to NUM_CELLS - 1 do
    begin
      ...
      if (cellMatrix[i, j] = 1) and (neighborCount < 2) then tempMatrix[i, j] := 0
      else
      if (cellMatrix[i, j] = 1) and (neighborCount > 3) then tempMatrix[i, j] := 0
      else
      if (cellMatrix[i, j] = 0) and (neighborCount = 3) then tempMatrix[i, j] := 1;
    end;
